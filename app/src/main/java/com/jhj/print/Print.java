...
public class Print {
	...
	public void inview() {
		print_dia = new Dialog(context, R.style.edit_AlertDialog_style);
		print_dia.setContentView(R.layout.activity_start_dialog_dtn_print);
		Pring_qd = (Button) print_dia.findViewById(R.id.print_qd);
		Pring_again = (Button) print_dia.findViewById(R.id.print_again);
		Pring_qd.setOnClickListener(new OnClickListener() {
		...
		Pring_again.setOnClickListener(new OnClickListener() {
		...
		if (su.getPrefboolean(context, "on_off", false)) {
			try {
				try {
					...
					json7.put("content", "终端号：" + clientId);
					json7.put("size", "2");
					...
					Bitmap qr = BitmapFactory.decodeResource(
							context.getResources(), R.drawable.edit_bg);

					Bitmap[] bitmaps = new Bitmap[] { qr.createScaledBitmap(qr,
							240, 240, true) };
					ServiceManager
							.getInstence()
							.getPrinter()
	...
	public void toast(String str) {
		Toast.makeText(context.getApplicationContext(), str, Toast.LENGTH_SHORT)
				.show();
	}
